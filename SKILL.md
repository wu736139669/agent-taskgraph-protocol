...
**关键规则**：
1. **仅当老板明确要求"新建会话/新线程"时才用 `create_thread`**（用户拥有的线程显示在侧边栏）；内部短暂并行用不可见子 agent，不制造用户侧会话
2. `create_thread` 是异步派发，**必须用 `wait_threads` 等待进展**
3. `wait_threads` 间隔默认 300s（省 token），活跃期加密——即第 8 节 wait 循环的 codex 原生形态
4. 循环退出条件不变（队列空/老板消息/循环失败/无进展）
5. claude/hapi 环境无这些工具时，回退到第 7/8 节通用机制（hapi 会话 + 外部守护）
