...
**关键规则**：
1. **`create_thread` 的决策（PMO 自己判断，不是一刀切）**：
   - **新建**：首次开工的任务、并行组隔离（互不相关）、失败重试（错误上下文不污染）、原线程上下文已耗尽
   - **复用**：同一模块连续迭代、同一需求多轮改动、流水线同链任务——用 `list_threads` 找到原线程，`send_message_to_thread` 继续，不新建
   - 判断依据：**任务相关性优先于"新任务就新线程"的习惯**——相关性（同模块/同需求/依赖链）决定复用，隔离/故障/首次才新建
2. 老板明确要求"新建会话"时按老板要求执行（用户拥有的线程显示在侧边栏）；内部短暂并行用不可见子 agent，不制造用户侧会话
