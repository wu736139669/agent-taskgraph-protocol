...
- 与项目已有长会话协议同理念（如 game 项目的 SESSION_CONTINUITY_PROTOCOL：聊天内容只能帮助定位，不得覆盖 durable 文件）
...
**协同断点防护（worker 完成但 PMO 不知道的僵局）**——会话是事件驱动的，没人叫它它就不动；worker 完成不会自动叫 PMO。防护四条：
1. **完成必报**：worker 完成 / 失败 / 卡住时，必须按 Goal 里的"报告协议"通知 PMO（`hapi ping-peer <PMO会话ID>`）+ 更新台账状态。**不通知 = 任务未完成**，验收闸不通过
2. **联系信息进 Goal**：派发时把 PMO 会话 ID 与通知方式写进 Goal 的分配记录——worker 永远知道活干完了找谁
3. **等待有归属**：任何"等待"状态在 ledger 写明**在等谁 / 等什么 / 静默多久算异常**——"worker 等 PMO 验收"这类状态必须一眼可识别，不能藏在对话里
4. **idle 兜底**：PMO 空闲（日志只有 UI 心跳、无实质工作）而台账有未处理完成事件 → 老板看 STATUS.md 的"最后更新"列即可发现异常，由老板唤醒或介入
