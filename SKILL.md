...
每条必须：能独立验收、能独立合并、不触碰被锁文件。拆不出独立块的 → 不需要多 agent，回到第 1 节。**拆完显式声明边**：每条 goal 的"依赖与边"字段写明依赖（上阶段产物 = 本任务输入）与并行组，同步更新 GRAPH.md。
...
4. 每条需求 = 一条简报 = 一个 worker（简报引用第 1 节分诊结论）。**分组后显式声明**：每条 goal 的"依赖与边"字段写入并行组号（G1/G2…）或串行依赖，同步更新 GRAPH.md
...
- `GRAPH.md`：任务图（格式见 `templates/GRAPH.md`，mermaid）——节点 = 活跃任务，边 = 依赖/交接（取自各 goal 的"依赖与边"字段）。派发/状态变化时更新；老板看图秒懂协作拓扑
...
**PMO 主动监控——开工强制机制（不是意愿）**：不主动看，PMO 就什么都不知道——信息只能来自 worker 报告（不可靠）、老板提醒（被动）、巧合。所以"主动看"必须落地为开工时的强制动作：
1. **开工第一步（强制）**：创建真实监控机制——Codex automation / Claude 后台任务 / 系统 cron，任选 PROJECT.md 本机环境的可用路径，间隔默认 60-120s
2. **登记证据（强制）**：机制标识（automation ID / 任务名 / cron 行）+ 间隔记入 PROJECT.md"本机环境"节——**写不出机制证据 = 未开工**（与 reviewer 证据同等级）
3. **周期动作**：每周期检查 ①worker 日志尾部（最后写入 + 新事件，`tail -c 2000`）——有新事件 = 在进展；长时间无新写入 = 静默告警；②台账（STATUS / ledger 状态与最后更新）——未处理完成事件、等待超时的任务 → 主动推进
4. **机制失效恢复**：机器重启 / automation 被删导致机制没了 → PMO 在任何交互中发现台账"最后更新"过期 → 重建机制并更新登记
5. 事件解读：CALL = 工具调用（进展信号）；AGENT = 实质产出；终态/错误 = 完成与异常
6. 诊断闭环：可疑模式 → 读日志上下文（`tail -60`）→ 判断正常/异常 → **误报也要记录**（下次不重复误判）
