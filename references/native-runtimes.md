...
| 逻辑 Team，Members 只需向 Lead 交付 | 原生 subagents，由 Lead 协调 | subagents，由 Lead 协调 |
| Members 必须互相讨论、认领共享 Tasks | 原生 subagents；Lead 转交 Handoff | Agent Teams |
...
Codex Team 默认是 `hub-and-spoke`：当前主线程是 Lead，原生 subagents 是 Members。不要把逻辑 Team 描述为 Members 可直接通信的 peer team，除非当前宿主明确暴露并验证了该能力。

- Team Charter 和任务图由 Lead 维护。
- Members 之间的依赖通过 Lead、代码产物或持久化 Handoff 传递。
- Lead 使用原生 task/plan 能力记录进度；不要创建第二套运行时状态机。

### 派发

- 使用当前环境提供的 `spawn_agent` 或等价原生工具；不要用 shell 再启动一个 Codex CLI。
- 优先使用宿主内置通用、Explorer、Worker 或 Monitor Agent 类型。只有 Role 会反复出现且默认类型不足时，才维护仓库或用户级自定义 Agent 配置。
- 每个 spawn 只对应一个边界清楚的 Task。独立 Tasks 可以并行启动；有依赖的 Task 等 Handoff 后再启动。
...
Agent Teams 适合 teammates 必须互相消息、共享 Task 列表、认领工作或协作决策的场景。若 Member 只需把结果交回 Lead，subagents 更简单且成本更低。
...
- 不把 Agent Team 设计成依赖 Lead resume 的持久会话。Lead 会话结束前先收集 Handoffs；恢复主会话后若 teammates 不存在，按 Handoff 重新创建。
